...
export default async function TeamMemberPage({
  ...
  return (
    <div className="min-h-screen py-12 px-4 max-w-5xl mx-auto">
      <Link
        href="/team"
        className="inline-flex items-center text-[#021A2B] font-medium mb-8 group"
      >
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 md:p-10 grid md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <p className="text-[#021A2B] font-semibold">{member.title}</p>
          <p className="text-sm text-gray-500">NMLS #{member.nmls}</p>

          <div className="flex flex-wrap gap-2 pt-2">
            <a
              href={`tel:${member.phone.replace(/\D/g, "")}`}
              className="inline-flex items-center gap-2 bg-[#F0F7F3] text-[#021A2B] px-4 py-2 rounded-full text-sm"
            >
          ...
          <p className="text-sm text-gray-600 flex items-center gap-2">
            <Icons.MapPin className="w-4 h-4 text-[#021A2B]" />
            {member.address}
          </p>
      ...
      <section className="mb-16 bg-[#021A2B] text-white rounded-2xl p-8 text-center">
        ...
        <p className="text-green-200 max-w-2xl mx-auto mb-8">
          It{`'`}s simple to get started on your mortgage journey – whether you
          {`'`}re just looking to get pre-qualified or are ready to submit an
